...
int bs(int A[], int n, int key, int low, int high) {
  ...
  if(low > high)
    ...
    return bs(A, n, key, 0, mid-1);
  else
    return bs(A, n, key, mid+1, n-1);
}

int FindItemSorted(int A[], int n, int key) {
  return bs(A, n, key, 0, n-1);
}
...
void FillArrayTrueRandom(int A[], int n) {
  int mods[10] = {4, 44, 444, 4444, 44444, 444444, 4444444, 44444444, 444444444, 4444444444};
  srand(time(0));
  for(int i = 0; i<n; i++)
